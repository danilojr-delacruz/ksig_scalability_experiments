...
def get_function_call_code(L, name):
    return f"""L = {L}
# Generate X, Y
key = jax.random.split(jax.random.PRNGKey(0), num=1)
X = jax.random.normal(key[0], shape=(N, L, d))
X = X.cumsum(axis=1) / jax.numpy.sqrt(L)
...
with open("sigkerax/base_script.py", "r") as f:
    base_script = f.read()
...
with open(f"{OUTPUT_DIR}/brownian_sigkerax.pkl", "wb") as f:
